...
import 'dart:convert';
import 'package:hrm_admin_app/Services/face_detector_service.dart';
...
class _CheckOutVerificationScreenState
  ...
  Future<void> _fetchLocationAndTime() async {
    ...
    try {
      ...
      try {
        ...
        if (mounted) setState(() => locationController.text = "Location found");
      }
    } catch (e) {
  ...
  Widget _vehicleDropdownField() {
    return SizedBox(
      height: 48,
      child: DropdownButtonFormField<String>(
        initialValue: selectedVehicleMode,
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Checked out successfully (Demo)'),
